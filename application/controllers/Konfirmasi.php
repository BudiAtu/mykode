	...
	function index(){
		...
		if (isset($_POST['submit'])){
			...
			if (isset($_POST['submit1']) OR $_GET['kode']){
				...
				//$data['record'] = $this->model_app->view('rb_reseller');
				$row = $this->db->query("SELECT a.id_penjualan, b.id_konsumen, a.id_penjual FROM `rb_penjualan` a jOIN rb_konsumen b ON a.id_penjual=b.id_konsumen where status_penjual='reseller' AND a.kode_transaksi='$kode_transaksi'")->row_array();
				$data['record'] = $this->model_app->view_where('rb_rekening_reseller',array('id_konsumen'=>$row['id_penjual']));
				
			
				$data['total'] = $this->db->query("SELECT sum(a.harga_jual-a.diskon) as total, a.id_penjualan FROM `rb_penjualan_detail` a where a.id_penjualan='".$row['id_penjualan']."'")->row_array();
