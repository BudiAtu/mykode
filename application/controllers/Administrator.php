...
class Administrator extends CI_Controller {
		...
		if (isset($_POST['submit'])){
            ...
            if ($this->session->level!=''){
              redirect($this->uri->segment(1).'/home');
                ...
                $this->session->set_userdata('mycaptcha', $cap['word']);
    			$data['title'] = 'Administrator &rsaquo; Log In';
    			$this->load->view('administrator/view_login_kontributor',$data);
            }
		}
	}
        ...
        if (isset($_POST['lupa'])){
            ...
            if ($cekemail <= 0){
                ...
                $message      = "<html><body>
                                ...
                                $config['smtp_user'] = "daganganku12@gmail.com"; // Isi dengan email anda
                                $config['smtp_pass'] = "i3udis4ntoso"; // isi dengan password email anda
	...
	function edit_komentarberita(){
		...
		if (isset($_POST['submit'])){
			...
			$data2 = array('isi_reply'=>$this->input->post('l'),
                        		'jam_reply'=>date('H:i:s'),
                        		'aktif'=>$this->input->post('d'),
                        		'tgl_reply'=>date('Y-m-d'),
					'isi_reply'=>$this->input->post('l'),
					'petugas'=>$this->session->username,
					'id_komentar'=>$this->input->post('id'));
			$this->model_app->insert('komentar_reply', $data2);
    ...
    function video(){
      ...
      function tambah_video(){
        ...
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/img_video/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|mp4|MP4';
    ...
    function delete_banner(){
     ...
     // Controller Modul banner_produk Link

     function banner_produk(){
        cek_session_akses('banner_produk',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('banner_produk','id_banner_produk','DESC');
        $this->template->load('administrator/template','administrator/mod_banner_produk/view_banner_produk',$data);
    }

    function tambah_banner_produk(){
        cek_session_akses('banner_produk',$this->session->id_session);
        if (isset($_POST['submit'])){
            $data = array('judul'=>$this->db->escape_str($this->input->post('a')),
                            'url'=>$this->input->post('b'),
                            'tgl_posting'=>date('Y-m-d'));
            $this->model_app->insert('banner_produk',$data);  
            redirect($this->uri->segment(1).'/banner_produk');
        }else{
            $this->template->load('administrator/template','administrator/mod_banner_produk/view_banner_produk_tambah');
        }
    }

    function edit_banner_produk(){
        cek_session_akses('banner_produk',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $data = array('judul'=>$this->db->escape_str($this->input->post('a')),
            ...
            $where = array('id_banner_produk' => $this->input->post('id'));
            $this->model_app->update('banner_produk', $data, $where);
            redirect($this->uri->segment(1).'/banner_produk');
        }else{
            $proses = $this->model_app->edit('banner_produk', array('id_banner_produk' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_banner_produk/view_banner_produk_edit',$data);
        }
    }

    function delete_banner_produk(){
        cek_session_akses('banner_produk',$this->session->id_session);
        $id = array('id_banner_produk' => $this->uri->segment(3));
        $this->model_app->delete('banner_produk',$id);
        redirect($this->uri->segment(1).'/banner_produk');
    }
	...
	function detail_pesanmasuk(){
		...
		if (isset($_POST['submit'])){
            ...
            $config = Array(  
                ...
                'smtp_port' => 465,  
                'smtp_user' => 'i3udi4tu@gmail.com',   
                'smtp_pass' => 'i3udis4ntoso',   
                'mailtype' => 'html',   
                'charset' => 'utf-8'  
               );  
            $this->load->library('email', $config);
            ...
            $this->email->to($email);
            $this->email->from('i3udi4tu@gmail.com','tokoku.COM');
            $this->email->subject($subject);
            $this->email->message($message);
            $this->email->send();
    ...
    // Controller Modul Rekening reseller

    function rekening_reseller(){
        cek_session_akses('rekening_reseller',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('rb_rekening_reseller','id_rekening_reseller','DESC');
        $this->template->load('administrator/template','administrator/additional/mod_rekening_reseller/view_rekening',$data);
    }

    function tambah_rekening_reseller(){
        cek_session_akses('rekening_reseller',$this->session->id_session);
        if (isset($_POST['submit'])){
            ...
            redirect('administrator/rekening_reseller');
        }else{
            $this->template->load('administrator/template','administrator/additional/mod_rekening_reseller/view_rekening_tambah');
        }
    }

    function edit_rekening_reseller(){
        cek_session_akses('rekening_reseller',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $data = array('nama_bank'=>$this->db->escape_str($this->input->post('a')),
                        'no_rekening'=>$this->db->escape_str($this->input->post('b')),
                        'pemilik_rekening'=>$this->db->escape_str($this->input->post('c')));
            $where = array('id_rekening_reseller' => $this->input->post('id'));
            ...
            $this->template->load('administrator/template','administrator/additional/mod_rekening_reseller/view_rekening_edit',$data);
        }
    }

    function delete_rekening_reseller(){
        cek_session_akses('rekening_reseller',$this->session->id_session);
    ...
    function pembayaran_konsumen(){
        ...
        $data['record'] = $this->db->query("SELECT a.*, b.*, c.kode_transaksi, c.proses FROM rb_konfirmasi_pembayaran_konsumen a JOIN rb_rekening_reseller b ON a.id_rekening=b.id_rekening_reseller JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where  c.status_penjual='reseller'");
