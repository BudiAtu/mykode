...
class Produk extends CI_Controller {
	function index(){
		...
		if (is_numeric($dari)) {
			if ($this->input->post('kata')){
				...
				$data['record'] = $this->db->query("SELECT * FROM rb_produk a LEFT JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen
	                                    where a.id_konsumen!='0' AND a.id_produk_perusahaan='0' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			}
	...
	function kategori(){
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT * FROM rb_produk a LEFT JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen
                                     where a.id_konsumen!='0' AND a.id_produk_perusahaan='0' AND a.id_kategori_produk='$cek[id_kategori_produk]' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
	...
	function subkategori(){
		...
		if (is_numeric($dari)) {
			...
			$data['keywords'] = keywords();
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_lengkap, c.nama_kota FROM rb_produk a LEFT JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_konsumen!='0' AND a.id_produk_perusahaan='0' AND a.id_kategori_produk_sub='$cek[id_kategori_produk_sub]' ORDER BY a.id_produk DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
			$this->template->load(template().'/template',template().'/reseller/view_kategori_all',$data);
		}else{
			redirect('main');
	...
	function reseller(){
		...
		if (is_numeric($dari)) {
			...
			if (isset($_POST['submit'])){
				...
				$data['record'] = $this->db->query("SELECT * FROM rb_konsumen a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id ORDER BY id_konsumen DESC LIMIT $dari,$config[per_page]");
			}
	...
	function detail_reseller(){
		$data['title'] = 'Detail Profile Member';
		$data['description'] = description();
		$data['keywords'] = keywords();
	...
	function produk_reseller(){
		$id = $this->uri->segment(3);
		$jumlah= $this->model_app->view_where('rb_produk',array('id_konsumen'=>$id))->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$data['keywords'] = keywords();
			$data['rows'] = $this->model_app->edit('rb_konsumen',array('id_konsumen'=>$id))->row_array();
			$data['record'] = $this->model_app->view_where_ordering_limit('rb_produk',array('id_konsumen'=>$id),'id_produk','DESC',$dari,$config['per_page']);
			$this->pagination->initialize($config);
	...
	function keranjang(){
		...
		$j = $this->model_reseller->jual_reseller($id_konsumen,$id_produk)->row_array();
        $b = $this->model_reseller->beli_reseller($id_konsumen,$id_produk)->row_array();
        $stok = $b['beli']-$j['jual'];
		...
		if ($id_produk!=''){
			if ($stok < $qty){
				...
				echo "<script>window.alert('Maaf, Stok untuk Produk $produk_cek pada Pelapak ini telah habis, silahkan menunggu atau order dengan pelapak lain!');
                                  window.location=('".base_url()."/produk/produk_reseller/$id_konsumen')</script>";
			}else{
				$this->session->unset_userdata('produk');
				if ($this->session->idp == ''){
					$idp = 'TRX-'.date('YmdHis');
					$this->session->set_userdata(array('idp'=>$idp,'reseller'=>$id_konsumen));
				}

				$cek = $this->model_app->view_where('rb_penjualan_temp',array('session'=>$this->session->idp,'id_produk'=>$id_produk))->num_rows();
				if ($this->session->reseller==$id_konsumen){
					if ($cek >=1){
					...
					if ($this->session->idp != ''){
						$data['rows'] = $this->model_app->edit('rb_konsumen',array('id_konsumen'=>$this->session->reseller))->row_array();
		...
		if (isset($_POST['submit'])){
				if ($this->session->idp!=''){
					...
					$data = array('kode_transaksi'=>$this->session->idp,
				        		  'id_pembeli'=>$id,
				        		  'id_penjual'=>$this->session->konsumen,
					...
					$res = $this->model_app->view_where('rb_konsumen',array('id_konsumen'=>$this->session->konsumen))->row_array();
					...
					$data['rekening_reseller'] = $this->model_app->view_where('rb_rekening_reseller',array('id_konsumen'=>$this->session->konsumen));
					...
					$message .= "<tr bgcolor='lightgreen'>
					      ...
					      <table style='width:100%;' class='table table-hover table-condensed'>
							...
							<tbody>";
							    $noo = 1;
							    $rekening = $this->model_app->view_where('rb_rekening_reseller',array('id_konsumen'=>$this->session->konsumen));
	...
	public function detail(){
		...
		$dat = $this->db->query("SELECT * FROM rb_produk where produk_seo='$ids' AND id_konsumen!='0'");
	    $row = $dat->row();
	    $total = $dat->num_rows();
	        if ($total == 0){
	        	redirect('main');
