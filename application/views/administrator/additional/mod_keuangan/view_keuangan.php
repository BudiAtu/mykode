              ...
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <?php 
                    ...
                    $pembelian = $this->db->query("SELECT sum((b.jumlah*b.harga_jual)-b.diskon) as total FROM rb_penjualan a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_penjual='admin' AND a.id_pembeli='$row[id_konsumen]' AND a.proses='1'")->row_array();
                    $penjualan_perusahaan = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk
                                                                JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='$row[id_konsumen]' AND c.proses='1'")->row_array();
                    ...
                    $total_jual = 0;
                    $total_bonus = 0;
                    $reseller = $this->db->query("SELECT * FROM rb_konsumen where referral='$row[username]'");
                      foreach ($reseller->result_array() as $rows) {
                        $pp = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk
                                                                  JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='".$rows['id_konsumen']."' AND c.proses='1'")->row_array();
                        $total_jual = $total_jual+$pp['total'];
                        $total_bonus = $total_bonus+($set['referral']/100*$pp['total']);
                      }

                    $pen = $this->db->query("SELECT sum(bonus_referral) as pencairan FROM rb_pencairan_bonus where id_konsumen='$row[id_konsumen]'")->row_array();
                    echo "<tr><td>$no</td>
                              ...
                              <td><center>
                                <a class='btn btn-success btn-xs' title='Detail Data' href='".base_url()."administrator/detail_reseller/$row[id_konsumen]'><span class='glyphicon glyphicon-search'></span> Detail</a>
                                <a class='btn btn-info btn-xs' title='Bayarkan Bonus Rerral' href='".base_url()."administrator/bayar_bonus/$row[id_konsumen]'><span class='glyphicon glyphicon-ok'></span> Bayar</a>
                              </center></td>
                          </tr>";
                      $no++;
                    }
