<p class='sidebar-title text-danger produk-title'>Detail Data Produk Pelapak</p>
  ...
  <tbody>
    ...
    <tr><th scope='row' width='100px'>Nama</th> <td><?php echo $rows['nama_lengkap']?></td></tr>
    <tr><th scope='row'>Alamat</th> <td><?php echo "Kota $rows[nama_kota], $rows[alamat_lengkap]"; ?></td></tr>
    <tr><th scope='row'>Kontak</th> <td><?php echo "Hp. <b style='color:green'>$rows[no_hp]</b>, Email. <i style='text-decoration:underline; color:blue'>$rows[email]</i>"; ?></td></tr>
  ...
  <hr>
      ...
      <?php 
        ...
        $disk = $this->model_app->edit('rb_produk_diskon',array('id_produk'=>$row['id_produk'],'id_konsumen'=>$this->uri->segment(3)))->row_array();
        ...
        echo "<div class='col-sm-4 produk'>
                  <center>
                    ...
                    if ($beli['beli']-$jual['jual']<=0){
                      ...
                      echo "<a class='btn btn-default btn-block btn-sm' href='".base_url()."members/keranjang/$rows[id_konsumen]/$row[id_produk]'>Beli Sekarang</a>";
                    }
