<?php
$rows = $this->db->query("SELECT * FROM `rb_konsumen` where id_konsumen='$record[id_konsumen]'")->row_array();
echo "<div class='col-md-12'>
    <div class='col-md-9' style='padding:0px'>
        ...
        $subkat = $this->model_app->view_where('rb_kategori_produk_sub',array('id_kategori_produk_sub'=>$record['id_kategori_produk_sub']))->row_array();
        $jual = $this->model_reseller->jual_reseller($record['id_konsumen'],$record['id_produk'])->row_array();
        $beli = $this->model_reseller->beli_reseller($record['id_konsumen'],$record['id_produk'])->row_array();
        ...
        <div class='col-md-9' style='padding:0px'>
            ...
            if ($this->session->level=='konsumen'){
                echo "<form action='".base_url()."members/keranjang/$record[id_konsumen]/$record[id_produk]' method='POST'>";
            }else{
                echo "<form action='".base_url()."produk/keranjang/$record[id_konsumen]/$record[id_produk]' method='POST'>";
            }
            echo "<table class='table table-condensed' style='margin-bottom:0px'>
                ...
                <a target='_BLANK' style='border-radius:15px 0px 0px 15px' class='btn btn-danger btn-sm pull-right' href='https://api.whatsapp.com/send?phone=+62$rows[no_telpon]&text=$record[nama_produk],... Apakah%20produk%20Ini%20bisa%20Nego?...'>Coba Nego Pelapak</a>
                </td></tr>
                <tr><td style='font-weight:bold; width:90px'>Demo</td> <td><a class='btn btn-danger' href='$record[url_demo]'>Demo</a></td></tr>
                <tr><td style='font-weight:bold'>Kategori</td> <td><a href='".base_url()."produk/kategori/$kat[kategori_seo]'>$kat[nama_kategori]</a></td></tr>
                <tr><td style='font-weight:bold'>Sub Kategori</td> <td><a href='".base_url()."produk/subkategori/$subkat[kategori_seo_sub]'>$subkat[nama_kategori_sub]</a></td></tr>";
        ...
        echo "</form><br><a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=+62$rows[no_hp]&text=$record[nama_produk],... Apakah%20Stok%20Masih%20ada?...'><span class='glyphicon glyphicon-comment'></span>  Apakah Stok Masih ada?</a>
            <a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=+62$rows[no_hp]&text=$record[nama_produk],... Saya%20Pesan%20Sekarang%20ya!'><span class='glyphicon glyphicon-comment'></span> Saya Pesan Sekarang ya!</a>
            <a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=+62$rows[no_hp]&text=Assalam,%20Haloo!%20$rows[nama_lengkap],%20Saya%20Mau%20Order%20Produknya...'><span style='color:green' class='glyphicon glyphicon-certificate'></span> Chat dengan Pelapak.</a>
        </div>
        </div>
