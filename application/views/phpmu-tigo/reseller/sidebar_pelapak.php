...
echo "<div class='widget'>
    <h3>Tentang Pelapak</h3>";
      ...
      <tbody>";
        ...
        $sukses = $this->db->query("SELECT * FROM rb_penjualan where id_penjual='$rows[id_konsumen]' AND status_penjual='reseller' AND proses!='0'");
        $pelanggan = $this->db->query("SELECT * FROM rb_penjualan where id_penjual='$rows[id_konsumen]' AND status_pembeli='konsumen' AND status_penjual='reseller' GROUP BY id_pembeli");
        ...
        <tr><td colspan='2'><b style='font-size:16px'>$rows[nama_lengkap]</b></td></tr>
        <tr><td colspan='2'><span class='glyphicon glyphicon-globe'></span> $rows[nama_kota]</td></tr>
        <tr><td colspan='2'><span class='glyphicon glyphicon-phone-alt'></span> $rows[no_hp]</td></tr>
      </tbody>
      </table>
</div>

<a class='btn btn-xs btn-block btn-success' href='".base_url()."produk/produk_reseller/$rows[id_konsumen]'>Semua Produk</a>
<a class='btn btn-xs btn-block btn-info' href='".base_url()."produk/detail_reseller/$rows[id_konsumen]'>Lihat Profile</a>
