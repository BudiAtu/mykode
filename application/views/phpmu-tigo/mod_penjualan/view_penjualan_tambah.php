            <div class="col-xs-12">  
              <div class="box">
                ...
                <?php 
                  ...
                  echo form_open_multipart('members/tambah_penjualan',$attributes); 
                ...
                ?>
                  ...
                  <a class='btn btn-default btn-sm' href='<?php echo base_url(); ?>members/penjualan'>Selesai / Kembali</a>
                  <hr>
                    ...
                    <?php 
                        echo "<tr>
                                ...
                                <td><select name='aa' class='combobox form-control' onchange=\"changeValue(this.value)\" autofocus>
                                                                      ...
                                                                      foreach ($barang as $r){
                                                                        $disk = $this->model_app->edit('rb_produk_diskon',array('id_produk'=>$r['id_produk'],'id_konsumen'=>$this->session->id_konsumen))->row_array();
                                ...
                                <td><button type='submit' name='submit' class='btn btn-success  btn-xs'><span class='glyphicon glyphicon-ok' aria-hidden='true'></span></button>
                                    <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url()."members/tambah_penjualan'><span class='glyphicon glyphicon-remove'></span></a>
                                </td>
                              </tr>";
                      ?>
                    <tbody>
                  <?php 
                    ...
                    $sub_total = ($row['harga_jual']);
                    echo "<tr><td>$no</td>
                              <td>$row[nama_produk]</td>
                              <td>Rp ".rupiah($row['harga_jual'])."</td>
                              <td>Rp ".rupiah($sub_total)."</td>
                              <td>
                                <a class='btn btn-warning btn-xs' title='Edit Data' href='".base_url()."members/tambah_penjualan/$row[id_penjualan_detail]'><span class='glyphicon glyphicon-edit'></span></a>
                                <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url()."members/delete_penjualan_tambah_detail/$row[id_penjualan_detail]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                              </td>
                          </tr>";
                      $no++;
                    }
                    
                    $total = $this->db->query("SELECT sum(a.harga_jual-a.diskon) as total FROM `rb_penjualan_detail` a where a.id_penjualan='".$this->session->idp."'")->row_array();
