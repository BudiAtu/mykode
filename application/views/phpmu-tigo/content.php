<aside id="block-5" class="widget widget_block"><p><a target="_BLANK" style="color:#fff; margin-top: -10px;" class="btn btn-primary btn-block" href="https://members.phpmu.com/auth/login"><b>Login</b></a>
...
<a target="_BLANK" style="color:#fff; margin-top: 5px;" class="btn btn-success btn-block" href="https://klikwa.net/phpmu.com-support"><b>Chat by Whatsapp</b></a>
...
foreach ($kategori->result_array() as $rows) {
	echo "<a class='btn btn-sm btn-danger' href='".base_url()."produk/kategori/$rows[kategori_seo]'>$rows[nama_kategori]</a>";
}
			...
			echo "<li style='width:180px'>
					<div class='article-photo'>
						<a href='".base_url()."$row[judul_seo]' class='hover-effect'>";
							if ($row['gambar'] ==''){
								echo "<a class='hover-effect' href='".base_url()."$row[judul_seo]'><img style='height:110px; width:200px' src='".base_url()."asset/foto_berita/no-image.jpg' alt='no-image.jpg' /></a>";
							}else{
								echo "<a class='hover-effect' href='".base_url()."$row[judul_seo]'><img style='height:110px; width:200px' src='".base_url()."/asset/foto_berita/$row[gambar]' alt='$row[gambar]' /></a>";
							}
					echo "</a>
					</div>
					<div class='article-content'>
						...
						<span class='meta'>
							<a href='".base_url()."$row[judul_seo]'><span class='icon-text'>&#128340;</span>$row[jam], $tgl,Dilihat $row[dibaca] Kali</a>
						</span>
					</div>
				  </li>";
			}
...
<div class='block'>
				<div class='block-title'>
					<a href='produk' class='right'>+ Produk Lainnya</a>
					<h2>PRODUK TERKINI</h2>
				</div>
				<div class='block-content'>";
			  $no = 1;
			  $record = $this->db->query("SELECT * FROM rb_produk where id_konsumen!='0' AND id_produk_perusahaan='0' ORDER BY id_produk DESC LIMIT 24");
			    ...
			    if ($beli['beli']-$jual['jual']<=0){ $stok = '<b style="color:red">Stok Habis</b>'; }else{ $stok =  "<span style='color:green'>Stok ".($beli['beli']-$jual['jual'])." $row[satuan]</span>"; }
...
//	$produk = $this->model_reseller->produk_perkategori(0,0,$kat['id_kategori_produk'],12);
//	    echo "<p class='sidebar-title text-danger produk-title'>$kat[nama_kategori]</p>
//	    <div class='container'>";
//	    foreach ($produk->result_array() as $row){
//	    $ex = explode(';', $row['gambar']);
//	    if (trim($ex[0])==''){ $foto_produk = 'no-image.png'; }else{ $foto_produk = $ex[0]; }
//	    if (strlen($row['nama_produk']) > 25){ $judul = substr($row['nama_produk'],0,15).',..';  }else{ $judul = $row['nama_produk']; }
//	    $jual = $this->model_reseller->jual_reseller($row['id_konsumen'],$row['id_produk'])->row_array();
//	    $beli = $this->model_reseller->beli_reseller($row['id_konsumen'],$row['id_produk'])->row_array();
