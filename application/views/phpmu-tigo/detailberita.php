						...
						<div class="article-title">
							<div class="share-block right">
								<div>
									...
									<div class="left">
										<script language="javascript">
										document.write("<a href='http://www.facebook.com/share.php?" + document.URL + " ' target='_blank' class='custom-soc icon-text'>&#62220;</a> <a href='http://twitter.com/home/?status=" + document.URL + "' target='_blank' class='custom-soc icon-text'>&#62217;</a> <a href='https://plus.google.com/share?url=" + document.URL + "' target='_blank' class='custom-soc icon-text'>&#62223;</a>");
										</script>
										<a href="#" class="custom-soc icon-text">&#62232;</a>
										<a href="#" class="custom-soc icon-text">&#62226;</a>
									</div>
...
$komentar = $this->model_utama->view_where_ordering_limit('komentar',array('id_berita' => $rows['id_berita'],'aktif' => 'Y'),'id_komentar','ASC',0,100);
	...
	echo "<div style='background:$warna' class='commment-content'>
		  ...
		  $komentar_reply = $this->model_utama->view_where_ordering_limit('komentar_reply',array('id_komentar' => $kka['id_komentar'],'aktif' => 'Y'),'id_komentar','ASC',0,100);
			foreach ($komentar_reply->result_array() as $kkb) {
			$isian=nl2br($kkb['isi_reply']); 
			$komentarku = sensor($isian); 
 
			if(($no % 2)==0){ $warna="#ffffff;"; }else{ $warna="#e3e3e3"; }
			$test = md5(strtolower(trim($kkb['email'])));	
			echo "<div style='background:$warna; margin-left:30px' class='commment-content'>
						<div class='user-avatar'>
					<a href='#' class='hover-effect'>";
					if ($kka['email'] == ''){
							echo "<img class='setborder' src='".base_url()."asset/foto_user/blank.png'/>";
				...
				echo "</a>
					...
					<strong class='user-nick'>$kkb[petugas]</a></strong>
					<span class='time-stamp'>".tgl_indo($kkb['tgl_reply']).", $kkb[jam_reply] WIB</span>
					<div class='comment-text'><p>$komentarku</p></div>
				  </div>";
			$no++;
		}
