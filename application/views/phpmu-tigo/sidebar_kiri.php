<div class="block">
	<h2 class="list-title" style="color: green;border-bottom: 2px solid green;">Kategori Produk</h2>
	<ul class="article-list">
		<?php
		  $banner = $this->model_utama->view_ordering_limit('banner_produk','id_banner_produk','ASC',0,5);
...
<div class="block">
	<?php $r = $this->model_utama->view_where('kategori',array('sidebar' => 0))->row_array(); ?>
	<h2 class="list-title" style="color: #dd8229;border-bottom: 2px solid #dd8229;">Berita <?php echo "$r[nama_kategori]"; ?></h2>
	<ul class="article-block">
	<?php 
		$kategori0 = $this->model_utama->view_join_two('berita','users','kategori','username','id_kategori',array('berita.id_kategori' => $r['id_kategori'],'berita.status' => 'Y'),'id_berita','DESC',0,5);			
		foreach ($kategori0->result_array() as $r2x) {
		...
		echo "<li>
					...
					<div class='article-content'>
						...
						<span class='meta'>
							<a href='".base_url()."$r2x[judul_seo]'><span class='icon-text'>&#128340;</span>$r2x[jam], ".tgl_indo($r2x['tanggal']).",Dibaca $r2x[dibaca] Kali</a>
						</span>
					</div>
				  </li>";
		}
